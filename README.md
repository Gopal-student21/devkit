...
Checks cover: hardcoded secrets & credential values (sk_/ghp_/AKIA/JWT), `eval`/command execution, SQL injection, XSS (`innerHTML`), TS `any`/non-null assertions, empty catches, ignored errors, debug logs, TODOs, magic numbers — plus AI-authored markers ("generated by Copilot") for attribution.
